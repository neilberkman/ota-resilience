...
Vulnerable OTA Bricks On Power Loss During Copy
    ${result}=    Run Process    python3    scripts/ota_fault_campaign.py    --scenario    vulnerable    --fault-range    0:28672    --fault-step    28672    --output    results/vulnerable_campaign.json    --renode-test    ${RENODE_TEST_BIN}    cwd=${ROOT}
...
Resilient OTA Recovers From Power Loss During Write
    ${result}=    Run Process    python3    scripts/ota_fault_campaign.py    --scenario    resilient    --fault-range    0:20000    --fault-step    20000    --output    results/resilient_campaign.json    --renode-test    ${RENODE_TEST_BIN}    cwd=${ROOT}
    Should Be Equal As Integers    ${result.rc}    0
...
Resilient OTA Recovers From Power Loss During Metadata Update
    ${result}=    Run Process    python3    scripts/ota_fault_campaign.py    --scenario    resilient    --include-metadata-faults    --fault-range    28160:28287    --fault-step    16    --output    results/resilient_meta_campaign.json    --renode-test    ${RENODE_TEST_BIN}    cwd=${ROOT}
    Should Be Equal As Integers    ${result.rc}    0
...
Resilient OTA Full Campaign
    ${result}=    Run Process    python3    scripts/ota_fault_campaign.py    --scenario    comparative    --fault-range    0:28160    --fault-step    14080    --output    results/full_campaign.json    --renode-test    ${RENODE_TEST_BIN}    cwd=${ROOT}
    Should Be Equal As Integers    ${result.rc}    0
