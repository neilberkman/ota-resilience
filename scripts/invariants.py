...
Usage from a campaign runner::
    ...
    violations = run_invariants(result, invariants=default_invariants("resilient"),
...
def default_invariants(scenario: str) -> List[InvariantFn]:
    ...
    Presets:
        ``"resilient"``:  All invariants.  The resilient protocol is
            expected to survive every single-fault scenario.
